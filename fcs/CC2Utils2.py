...
class OwnResults:
    ...
    def __init__(
        self, path, cc2results=None, counttimes=None, mintau=.05, maxtau=1.e3,
    ...
    def getraw(self, i):
        ...
        if self.raws[i] == None:
            with open(self.getrawpaths()[i], "rb") as f:
                self.raws[i] = np.fromfile(
                    f, '<u2')[30:]  # omit ConfoCor 2 text
        return self.raws[i]
    ...
    def getparams(self, i, loadacf=False, saveacf=False, acfsuffix=None,
        ...
        if self.acfs == None or self.acfs[i] == None or self.acfs[i].datastart != self.datastart \
           or self.acfs[i].dataend != self.dataend:
            self.getacf(
                i, loadacf=loadacf, saveacf=saveacf, acfsuffix=acfsuffix,
                verbose=verbose)
        guess = self.cc2results.getparams()[i]
        self.fits[i] = Fit(
            self.getrawpaths()[i] + fitsuffix, self.acfs[i], func, guess, fixed, fitstart)
        return self.fits[i].getparams(loadfit=loadfit, savefit=savefit, verbose=verbose)
    ...
    def getacf(self, i, loadacf=False, saveacf=False, acfsuffix=None,
        ...
        if loadacf and os.path.isfile(acfpath):  # load old acf
            ...
            countDwellInds = CC2UtilsX.getCountDwellInds(
                counttimes, self.dwell)
            ...
            # compute acf
            ac, ac_sd = CC2UtilsX.getAcf(
                maxTauDwell, countDwellInds, dwellCount)[minTauDwell - 1:]
            lags = list(btinus * self.dwell * (minTauDwell + x)
                        for x in range(len(ac)))
            acf = Acf(lags, ac, True, ac_sd=ac_sd)
            acf.mintau = self.mintau
            acf.maxtau = self.maxtau
            acf.datastart = self.datastart
            acf.dataend = self.dataend
            acf.dwell = self.dwell
            acf.fromfile = False
    ...
    def getcounttimes(self, i, verbose):
        ...
        if self.counttimes[i] == None:
            self.counttimes[i], cyclecount = CC2UtilsX.getCountTimes(
                self.getraw(i),
                verbose)
        datastartind, dataendind = CC2UtilsX.getCountTimeIndsFromSeconds(
            self.datastart,
            self.dataend,
            btinus,
            self.counttimes[i])
    ...
    def getsignaltonoiseratio(self, i, func=funcs.triplet, loadacf=False, saveacf=False, acfsuffix=None):
        ...
        sd = self.getsd(i, func=func, loadacf=loadacf,
                        saveacf=saveacf, acfsuffix=acfsuffix)[1][0]
        signal = self.getsignal(
            i, func=func, loadacf=loadacf, saveacf=saveacf, acfsuffix=acfsuffix)[0]
        return signal / sd
...
class CC2Results:
    ...
    def getacfs(self):
        ...
        paths = filter(os.path.isfile, [os.path.join(self.path, s)
                       for s in os.listdir(self.path)])
        try:
            for p in paths:
                if p[-4:] == '.fcs':
                    xFull_yFull.append(
                        np.genfromtxt(p, delimiter=', ', skip_header=16,
        ...
        for xFull, yFull in xFull_yFull:
            ...
            lags = xFull[:countRateStart] * \
                1e3  # from milliseconds to microseconds
    ...
    def getparams(self):
        ...
        try:
            rawparams = np.genfromtxt(
                tablepath, skip_header=1, delimiter='\t', dtype=None)
        except IOError:
        ...
        for i, name in enumerate(loadednames):
            if name.rfind('[') > -1:
                name = name[
                    :name.rfind('[') - 1]  # omit units, e.g. "[ us ]" (greek letters are dull)
...
class Fit:
    ...
    def getparams(self, loadfit=False, savefit=False, verbose=True):
        ...
        self.params = self.guess + Parameters.fromvalues(
            self.func, self.output.beta, self.output.sd_beta)
        return self.params
    ...
    def fitacf(self, loadfit=False, savefit=False, fitsuffix='fit.pickle', verbose=True):
        if loadfit and os.path.isfile(self.path):
            ...
            data = odr.RealData(x=self.acf.lags[self.fitstart:], y=ac[self.fitstart:], sx=lags_sd[self.fitstart:], sy=ac_sd[
                                self.fitstart:])  # sx=len(acfx)*[1e-99], sy=np.sqrt(yWeight)**-1)# sy=len(acfy)*[1e-99])
            model = odr.Model(self.func)
            fit = odr.ODR(data, model, self.guess.getfitvalues(
                self.func), ifixb=self.getfixed(), maxit=5000, job=10)
            self.output = fit.run()
            # save pickle
