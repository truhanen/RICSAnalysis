...
'''Helper class for performing and visualizing RICS analysis from confocal 2D images'''
...
lib_path = os.path.abspath(
    '/home/tutaruha/documents/work/Opiskelu/Kurssit/FYSZ490 Pro gradu/data/koodi/rics')
...
class Measurement:
    ...
    '''Class for easy RICS measurement data (confocal 2D image) processing and analysis.'''
    ...
    def __init__(self, path, ntaus=None, maximg=None, simpath=None,
                 ...
                 detectormeas=None):
        '''Parameters: path: image (.tif or .nd2) file path,
...
#                       *np.exp(-(self.pxsize*x/p[3])**2/(1.+4.*1.e-6*p[2]*x*self.dwell/p[3]**2))
    ...
    def getacnorm(self):
        ...
        if self._acnorm == None:
            if self.detectormeas != None:  # desperate try of detector correlation correction
                _, detectoracunnorm = self.detectormeas.getfitunnorm(
                    x=np.arange(self.getntaus()))
                acunnorm = [
                    acunnormi - detectoracunnorm for acunnormi in self.getacunnorm()]
                norm = self.getnorm()
                self._acnorm = [normed - 1 for normed in acunnorm / norm]
            else:
                self._acnorm = ricsx.getacnorm(
                    self.getntaus(), self.corrmethod,
    ...
    def getfitres(self):
        ...
        resy = np.array([acy[xi] - self.acf(params, acx[xi])
                        for xi in range(len(acx))])
        return acx, resy
    ...
    def getfitparams(self):
        ...
        if self._fitparams == None or self._fiterrors == None or self._fitteddata != self.fitdata:
            ...
            fit = odr.ODR(
                data, model, self.guess, ifixb=self.fixed, maxit=1000, job=10)
            output = fit.run()
            ...
            if self.guess != None:
                ...
                for i in range(len(self._fitparams)):
                    print ("   p[{0}] = {1:10.5g} +/- {2:10.5g}" +
                        ...
                        format(
                            i, self._fitparams[i], self._fiterrors[i], self.guess[i])
            if False:
                print "\nCorrelation Matrix :",
    ...
    def plot(
        self, cors=True, simfcs=True, cython=True, sd=True, fit=True, spline=False,
